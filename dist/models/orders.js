...
class OrdersStore {
    ...
    async addToCart(username, productId, orderId, quantity) {
        try {
            const conn = await database_1.default.connect();
            const sql = 'insert into order_products (username,quantity,order_id,product_id) values($1,$2,$3,$4) returning *';
            const result = await conn.query(sql, [
                username,
                quantity,
                orderId,
